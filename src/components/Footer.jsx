  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className='flex-1 flex flex-col justify start mr-10'>
        ...
        <p className={`${styles.paragraph} mt-4 max-w-[310px]`}>A new way to make</p>
      </div>
          ...
          <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
            ...
            <ul className='list-none mt-4'> {footerLink.links.map((link, index)=>(
              <li key={link.name} className={`
                ...
                cursor-pointer
                ${index !== footerLink.links.length-1 ? 'mb-4' : 'mb-0'}`}>{link.name}
              </li>
            ))} </ul>
          </div>
        ))}
    ...
    <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 
      ...
      <p className='
        ...
        text-[18px]
        leading-[27px]
        text-white'>2021 Hoobank, All Rights Reserved</p>
